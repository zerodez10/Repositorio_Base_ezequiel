...
Fabrica = sqlite3.connect("fabrica_programadores.db")

cursor = Fabrica.cursor()

# cursor.execute("CREATE TABLE Alunos (id INTEGER PRIMARY KEY AUTOINCREMENT, Nome Text, Idade integer, Cidade)")

# cursor.execute("CREATE TABLE Professores (id INTEGER PRIMARY KEY AUTOINCREMENT, Nome Text, Idade integer)")

# cursor.execute("CREATE TABLE Cursos (id INTEGER PRIMARY KEY AUTOINCREMENT, Nome text, Carga_horaria Integer, Preço Real)")

# cursor.execute("CREATE TABLE Matriculas (id INTEGER PRIMARY KEY AUTOINCREMENT, id_aluno INTEGER, id_curso INTEGER, id_professor INTEGER, data TEXT, FOREIGN KEY (id_aluno) REFERENCES Alunos(id), FOREIGN KEY (id_curso) REFERENCES Cursos(id), FOREIGN KEY (id_professor) REFERENCES Professores(id))")
